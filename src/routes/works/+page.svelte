<script lang="ts">
	...
	import type { SanityFile, SanityImage } from '$lib/types';
	...
	const selectedWork = $derived(selectedIndex !== null ? filteredWorks[selectedIndex] : null);
...
<div class="container">
	...
	{#await promise}
		<p>Loading...</p>
	{:then}
		<div class="tags-filter" use:animateIn={{ y: 20, delay: 0.1 }}>
			<button
				class="tag"
			...
			<button
				class="tag"
				class:active={selectedCategory === 'static'}
				onclick={() => handleCategoryClick('static')}
			>
				Статика
			</button>
		</div>

		<div class="tags-filter" use:animateIn={{ y: 20, delay: 0.2 }}>
		...
		<div class="gallery-grid">
			{#each filteredWorks as work, index (work._id)}
				<div animate:flip={{ duration: 300 }}>
					<button type="button" class="card-button" onclick={() => (selectedIndex = index)}>
						<WorkCard title={t(work.title)} tags={work.tags} thumbnailUrl={work.thumbnailUrl} />
					</button>
				</div>
			{/each}
		</div>
